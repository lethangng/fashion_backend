                            ...
                            <div class="card-body">
                                <div class="col-sm-12">
                                    <div class="row hidden-xs">
                                        <!-- App Search-->
                                        <form class="d-none d-lg-block col-sm-6" action="{{ route('evaluate.search') }}"
                                            method="GET">
                                            <div class="row">
                                    ...
                                    <table class="table align-middle table-nowrap mb-0 table-hover table-data">
                                        <thead class="table-light">
                                            <tr>
                                                ...
                                                <th class="align-middle">Xếp hạng</th>
                                        ...
                                        <tbody>
                                            @foreach ($evaluates as $evaluate)
                                                <tr>
                                                    <td>
                                                        <div class="form-check font-size-16">
                                                            <input class="form-check-input checkbox" type="checkbox"
                                                                id="{{ $evaluate['id'] }}" value="{{ $evaluate['id'] }}">

                                                        </div>
                                                    </td>
                                                    <td>
                                                        <img src="{{ $evaluate['product_image'] }}" alt="Hình ảnh sản phẩm">
                                                    </td>
                                                    <td>
                                                        {{ $evaluate['product_name'] }}
                                                    </td>

                                                    <td>
                                                        {{ $evaluate['star_number'] }} <i class="fas fa-star"
                                                            style="color: gold;"></i>
                                                    </td>
                                                    <td>
                                                        {{ $evaluate['content'] }}
                                                    ...
                                                    <td>
                                                        {{ date('d-m-Y', strtotime($evaluate['created_at'])) }}

                                                    </td>
                                                    <td class="text-center">
                                                        <form action="{{ route('evaluate.update') }}" method="POST">
                                                            @csrf
                                                    ...
                                                    <form action="{{ route('evaluate.delete') }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="id"
                                                            value="{{ $evaluate['id'] }}">
                                                        <td class="text-center">
                                                            <button type="button" class="btn btn-danger btn-sm"
                                                                ...
                                                                data-bs-target="#exampleModal{{ $evaluate['id'] }}"
                                                                data-bs-whatever="@mdo">
                                                            ...
                                                            <div class="modal fade"
                                                                id="exampleModal{{ $evaluate['id'] }}" tabindex="-1"
                                                                aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                                <div class="modal-dialog">
                                                                    <div class="modal-content">
                                                                        <div class="modal-header">
                        ...
                        <ul class="pagination pagination-rounded justify-content-center mt-1 mb-4 pb-1">
                            @for ($i = 1; $i < $total_pages + 1; $i++)
                                <li @class(['page-item', 'disabled' => $current_page == 1])>
                                    <a href="{{ route('evaluate.index', $i - 1) }}" class="page-link"><i
                                            class="mdi mdi-chevron-left"></i>
                                    </a>
                                </li>

                                <li @class(['page-item', 'active' => $i == $current_page])>
                                    <a href="{{ route('evaluate.index', $i) }}"
                                        class="page-link">{{ $i }}</a>
                                </li>
